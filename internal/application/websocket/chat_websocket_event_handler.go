...
func (h *ChatWebsocketEventHandler) createMessage(conn connection.Connection, data []byte) error {
	...
	msg, err := message.Create(request.Text, currentChatId.(uint64), conn.GetUser())
	if err != nil {
		return err
	}

	if _, err := h.messageService.Save(context.Background(), msg); err != nil {
		return err
	}
...
func NewMessageEventHandler(messageService services.MessageService) connector.EventHandler {
